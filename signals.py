...
MIN_TRADE_SCORE = 4  # this is a fallback only — settings["signal_threshold"] is the live value
...
class SignalEngine:
    ...
    def analyze(self, asset: str = "XAUUSD", settings: dict | None = None):
        ...
        # ── Daily candles → CPR levels (no caching — always fetch fresh) ─
        ...
        # debug log: shows exactly what candle the bot acted on every cycle.
        ...
        if _h1_filter:
            _h1_period = int((settings or {}).get("h1_ema_period", 50))
            ...
            if len(h1_closes) >= _h1_period:
                ...
                #  FAIL-SAFE: candle data empty or insufficient — mark as failed.
        ...
        #  FIX: Added 0.15% buffer zone — price must be clearly above/below
        ...
        # not blocked: price was hovering just above the H4 EMA (h4_ema_period, default 50).
        ...
        if _h4_filter:
            _h4_period = int((settings or {}).get("h4_ema_period", 50))
            ...
            if len(h4_closes) >= _h4_period:
                ...
                #  FAIL-SAFE: candle data empty — mark failed, block below.
        ...
        # ── 1b. H1 trend filter ( /  fail-safe) ─────────────────
        # Block trades that go against the H1 EMA trend.
        # BUY blocked if H1 price < H1 EMA (h1_ema_period, default 50) (bearish trend).
        # SELL blocked if H1 price > H1 EMA (h1_ema_period, default 50) (bullish trend).
        # if candle fetch failed (API timeout/rate-limit), block ALL trades
        ...
        # ── 1c. H4 trend filter ( /  fail-safe) ─────────────────
        # Macro hard block — H4 EMA (h4_ema_period, default 50) must agree with trade direction.
        # same fail-safe as H1 — block if data unavailable.
        if _h4_data_failed:
        ...
        if atr_val and atr_val > 0:
            ...
            if stretch > exhaustion_atr_mult:
                # hard block for extended setups (S2/R2) when exhausted.
        ...
        # 3.  fix: R1/S1 too close (< 0.50%) — fall back to fixed 0.75% TP
        ...
        if 0.50 <= level_dist_pct <= 0.75:
            ...
            # R1/S1 too close — fall back to fixed 0.75% TP ( fix)
            tp_pct_used = 0.75
